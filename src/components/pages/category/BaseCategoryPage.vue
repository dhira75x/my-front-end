        ...
        <div class="w-full lg:w-64 flex-shrink-0">
          <div class="p-6 bg-white rounded-lg shadow-md animate__animated animate__fadeIn">
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2">
                <div v-for="subCategory in subCategories" :key="subCategory.name" class="flex items-center">
                  <input type="checkbox" :id="`category-${subCategory.name}`" :value="subCategory.name"
                    v-model="selectedCategories"
                    class="w-4 h-4 text-lime-600 border-gray-300 rounded focus:ring-lime-500">
                  <label :for="`category-${subCategory.name}`" class="ml-2 text-sm text-gray-700">{{ subCategory.name
                  }}</label>
                </div>
              </div>
            </div>
            ...
            <div class="mb-6">
              ...
              <div class="space-y-4">
                <div>
                  <input type="range" id="minPrice" v-model="priceRange.min" min="0" max="1000000" step="1000"
                ...
                <div class="flex justify-between">
                  <div>
                    ...
                    <input type="number" id="minPriceInput" v-model="priceRange.min"
                  ...
                  <div>
                    ...
                    <input type="number" id="maxPriceInput" v-model="priceRange.max"
                      class="w-full px-3 py-1 text-sm border border-gray-300 rounded focus:ring-lime-500 focus:border-lime-500">
                  </div>
                </div>
              </div>
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2">
                <div v-for="brand in brands" :key="brand.id" class="flex items-center">
                  <input type="checkbox" :id="`brand-${brand.id}`" :value="brand.id" v-model="selectedBrands"
                    class="w-4 h-4 text-lime-600 border-gray-300 rounded focus:ring-lime-500">
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2">
                <div v-for="rating in ratings" :key="rating.value" class="flex items-center">
                  <input type="radio" :id="`rating-${rating.value}`" :value="rating.value" v-model="selectedRating"
                  ...
                  <label :for="`rating-${rating.value}`" class="ml-2 flex items-center text-sm text-gray-700">
                    <div class="flex text-yellow-400">
                      <svg v-for="i in 5" :key="i" class="w-4 h-4"
                        :class="i <= rating.value ? 'fill-current' : 'text-gray-300'" viewBox="0 0 24 24"
            ...
            <div class="mb-6">
              ...
              <div class="space-y-2">
                ...
                <div class="flex items-center">
                  <input type="checkbox" id="freeShipping" v-model="freeShipping"
                    class="w-4 h-4 text-lime-600 border-gray-300 rounded focus:ring-lime-500">
        ...
        <div class="flex-1">
          ...
          <div class="flex flex-col justify-between mb-6 sm:flex-row">
            ...
            <div class="flex items-center space-x-4">
              ...
              <div class="flex">
                <button @click="viewMode = 'grid'" class="p-2 text-gray-600 rounded-l hover:bg-gray-100"
                ...
                <button @click="viewMode = 'list'" class="p-2 text-gray-600 rounded-r hover:bg-gray-100"
          ...
          <div v-if="filteredProducts.length > 0">
            <div v-if="viewMode === 'grid'" class="relative overflow-hidden">
              <div class="flex transition duration-500 ease-in-out"
                :style="{ transform: `translateX(-${currentSlide * 100}%)` }">
                <div v-for="(slide, sIdx) in slides" :key="sIdx" class="flex-shrink-0 w-full">
                  <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                    <div v-for="product in slide" :key="product.id"
                      ...
                      <div>
                        <div class="relative">
                          ...
                          <div v-if="product.isNew"
                            class="absolute px-2 py-1 text-xs font-bold text-white rounded top-3 left-3 bg-lime-500">NEW
                          </div>
                          <div v-if="product.onSale"
                            class="absolute px-2 py-1 text-xs font-bold text-white rounded top-12 left-3 bg-red-500">
                            SALE</div>
                        </div>
                      </div>
                      <div class="p-4">
                        ...
                        <div class="flex items-center justify-between">
                          ...
                          <button @click.stop="addToCart(product)"
                            class="p-2 text-white transition rounded-full bg-lime-500 hover:bg-lime-600 duration-200 hover:scale-105 focus:ring-2 focus:ring-lime-400">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                              xmlns="http://www.w3.org/2000/svg">
                              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
              ...
              <div class="flex justify-center mt-6 space-x-2">
                <button v-for="(_, index) in slides" :key="index" @click="goTo(index)" class="w-3 h-3 rounded-full"
                  :class="currentSlide === index ? 'bg-lime-600' : 'bg-gray-300'"></button>
              </div>
              <button @click="prev"
                class="absolute left-2 top-1/2 -translate-y-1/2 p-2 bg-white bg-opacity-60 rounded-full hover:bg-opacity-90">◀</button>
              <button @click="next"
                class="absolute right-2 top-1/2 -translate-y-1/2 p-2 bg-white bg-opacity-60 rounded-full hover:bg-opacity-90">▶</button>
            </div>
              ...
              <div v-for="product in sortedProducts" :key="product.id"
                ...
                <div class="w-1/3 flex-shrink-0">
                  <div class="relative">
                    ...
                    <div v-if="product.isNew"
                      class="absolute px-2 py-1 text-xs font-bold text-white rounded top-3 left-3 bg-lime-500">NEW</div>
                    <div v-if="product.onSale"
                      class="absolute px-2 py-1 text-xs font-bold text-white rounded top-12 left-3 bg-red-500">SALE
                    </div>
                  </div>
                </div>
          ...
          <div v-if="filteredProducts.length > 0" class="flex items-center justify-between mt-8">
            <div class="text-sm text-gray-700">
              Showing <span class="font-medium">1</span> to <span class="font-medium">12</span> of <span
                class="font-medium">{{ filteredProducts.length }}</span> results
            </div>
            <div class="flex space-x-2">
              ...
              <button
                class="px-3 py-1 text-sm font-medium text-white bg-lime-600 border border-transparent rounded-md hover:bg-lime-700">
                1
              </button>
              <button
...
const filteredProducts = computed(() => {
  return props.products.filter(product => {
    ...
    // Category filter
    if (selectedCategories.value.length > 0 && !selectedCategories.value.some(category => product.categoryIds.includes(category))) {
      return false;
    }
