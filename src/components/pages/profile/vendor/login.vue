<template>
  <div class="max-h-screen">
    <section class="border-lime-500 bg-white min-h-screen flex items-center justify-center">
      <div class="bg-white p-5 flex rounded-2xl shadow-lg max-w-3xl">
        <div class="md:w-1/2 px-5 border-r-2 border-lime-500">
          <h2 class="text-2xl font-bold text-lime-900">Login</h2>
          <p class="text-sm mt-4 text-lime-800">If you have an account, please login</p>
          <form class="mt-6" @submit="handleSubmit">
            <div>
              <label class="block text-lime-900">Login ID</label>
              <input v-model="loginId" type="text" placeholder="Enter Login Id"
            ...
            <button type="submit"
              class="w-full block bg-lime-700 hover:bg-lime-500 focus:bg-lime-500 text-white font-semibold rounded-lg px-4 py-3 mt-6">
              Log In
            </button>
            <div class="mt-4 text-center">
              <p class="text-sm">
                Don't have an account yet?
                <router-link to="/registration" class="mx-2 text-lime-700 hover:underline">
        ...
        <div class="w-1/2 md:block hidden">
          <img src="../../../../assets/sideven.png" class="mt-28 bg-white" alt="page img" />
        </div>
      </div>
    </section>
  </div>
...
const login = async () => {
  ...
  try {
    ...
    if (response.status === "OK") {
      ...
      if (response.payload.isNinVerified) {
        window.location.href = "https://vendor-dashboard-seven-coral.vercel.app/";
      } else {
        alert("Your account is pending NIN verification. Please contact support.");
      }
    } else {
    ...
    const msg = error.response?.data?.msg || "Invalid credentials or server error";
    alert(msg);
  } finally {
