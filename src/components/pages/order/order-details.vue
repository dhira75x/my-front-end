        ...
        <div class="container px-4 mx-auto">
          <div class="max-w-3xl mx-auto">
            <div v-if="loading" class="animate-pulse">
              <!-- Skeleton Header -->
              <div class="mb-8">
                <div class="w-48 h-8 mb-2 bg-gray-200 rounded"></div>
              ...
              <div class="p-6 mb-8 bg-white rounded-lg shadow-md">
                <div class="flex flex-col justify-between md:flex-row md:items-center">
                  <div>
                    <div class="w-32 h-6 mb-2 bg-gray-200 rounded"></div>
                    <div class="w-40 h-4 bg-gray-200 rounded"></div>
                  </div>
                  <div class="w-24 h-8 mt-4 bg-gray-200 rounded-full md:mt-0"></div>
                </div>
              </div>
              ...
              <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                ...
                <div class="p-6 bg-white rounded-lg shadow-md">
                  <div class="w-32 h-6 mb-6 bg-gray-200 rounded"></div>
                  <div class="space-y-6">
                    <div v-for="i in 2" :key="i" class="flex pb-4 border-b border-gray-100 last:border-0 last:pb-0">
                      <div class="flex-shrink-0 w-20 h-20 bg-gray-200 rounded-lg"></div>
                      <div class="flex-1 ml-4">
                        <div class="w-3/4 h-5 mb-2 bg-gray-200 rounded"></div>
                        <div class="w-1/4 h-4 mb-2 bg-gray-200 rounded"></div>
                ...
                <!-- Skeleton Shipping & Payment -->
                <div class="space-y-6">
                  <div class="p-6 bg-white rounded-lg shadow-md">
                    <div class="w-40 h-6 mb-4 bg-gray-200 rounded"></div>
                    <div class="space-y-2">
                      <div class="w-1/2 h-4 bg-gray-200 rounded"></div>
                  ...
                  <div class="p-6 bg-white rounded-lg shadow-md">
                    ...
                    <div class="flex items-center">
                      <div class="w-10 h-10 mr-3 bg-gray-200 rounded-md"></div>
                      <div class="flex-1">
                        <div class="w-1/2 h-4 mb-2 bg-gray-200 rounded"></div>
              ...
              <div class="p-6 mt-8 bg-white rounded-lg shadow-md">
                <div class="w-32 h-6 mb-8 bg-gray-200 rounded"></div>
                <div class="relative flex justify-between">
                  <div v-for="i in 4" :key="i" class="flex flex-col items-center">
                    <div class="z-10 w-10 h-10 bg-gray-200 rounded-full"></div>
                    <div class="w-16 h-4 mt-2 bg-gray-200 rounded"></div>
                  </div>
                </div>
              </div>
            </div>
              ...
              <div class="grid grid-cols-1 gap-8 md:grid-cols-2">
                ...
                <div class="p-6 bg-white rounded-lg shadow-md">
                    ...
                    <div v-for="item in orderItems" :key="item.id"
                      ...
                      <div v-if="orderStatus.toLowerCase() === 'completed'"
                        ...
                        <div class="flex items-center justify-between mb-3">
                          <span class="text-sm font-bold text-gray-700 uppercase tracking-wider">Rate this
                            product</span>
                  ...
                  <div class="mt-6 pt-4 border-t border-gray-200">
                    ...
                    <div class="flex justify-between text-sm text-gray-600">
                      <span>Shipping</span>
                      <span>₦{{ 1000 * orderItems.length }}</span>
                    </div>
                    <div class="flex justify-between text-sm text-gray-600">
                      <span>Tax</span>
                      <span>₦{{ taxVal * subtotalVal }}</span>
                    </div>
...
const orderStatuses = computed(() => {
  ...
  const currentStatusIndex = statuses.findIndex(s => s.status.toLowerCase() === orderStatus.value?.toLowerCase());

  return statuses.map((s, index) => ({
    name: s.name,
    completed: currentStatusIndex >= index && orderStatus.value?.toLowerCase() !== 'cancelled',
    date: currentStatusIndex >= index ? (index === 0 ? orderDate.value : null) : null
  }));
});

const progressWidth = computed(() => {
  if (orderStatus.value?.toLowerCase() === 'cancelled') return 0;
  const steps = ['pending', 'processing', 'shipped', 'completed'];
  const index = steps.indexOf(orderStatus.value?.toLowerCase());
  if (index === -1) return 0;
...
const fetchOrderDetails = async () => {
  try {
    ...
    totalVal.value = data.total + (data.tax * data.total || 0) + (1000 * orderItems.value.length);
